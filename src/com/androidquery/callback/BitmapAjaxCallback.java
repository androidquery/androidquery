...
public class BitmapAjaxCallback extends AbstractAjaxCallback<Bitmap, BitmapAjaxCallback> {
    ...
    private static Bitmap decode(String path, byte[] data, BitmapFactory.Options options, int rotation) {
        ...
        return getRotatedBitmap(rotation, result);
    }

    private static Bitmap getRotatedBitmap(int rotation, Bitmap result) {
        if (result == null) {
            return result;
        }
     ...
     */
    public static Bitmap getResizedImage(String path, byte[] data, int target, boolean width, int round, int rotation) {
        ...
        if (target > 0) {
            ...
            decode(path, data, info, rotation);
        ...
        Bitmap bm = null;
        try {
            bm = decode(path, data, options, rotation);
    ...
    private Bitmap bmGet(String path, byte[] data, int rotation) {
        return getResizedImage(path, data, targetWidth, targetDim, round, rotation);
    ...
    protected Bitmap fileGet(String url, File file, AjaxStatus status) {
        try {
            int orientation = getRotation(file);
            return bmGet(file.getAbsolutePath(), null, orientation);
        } catch (IOException e) {
            AQUtility.report(e);
            return bmGet(file.getAbsolutePath(), null, 0);
        }
    }
    ...
    public Bitmap transform(String url, byte[] data, AjaxStatus status) {
        ...
        try {
            ...
            bm = bmGet(path, data, rotation);
        } catch (IOException e) {
            AQUtility.report(e);
            bm = bmGet(path, data, 0);
        }
