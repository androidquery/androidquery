...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants {
    ...
    public Bitmap getCachedImage(String url, int targetWidth) {
        ...
        if (result == null) {
            ...
            if (file != null) {
                try {
                    int rotation = BitmapAjaxCallback.getRotation(file);
                    result = BitmapAjaxCallback.getResizedImage(file.getAbsolutePath(), null, targetWidth, true, 0, rotation);
                } catch (IOException e) {
                    AQUtility.report(e);
                    result = BitmapAjaxCallback.getResizedImage(file.getAbsolutePath(), null, targetWidth, true, 0, 0);
                }
            }
        }
